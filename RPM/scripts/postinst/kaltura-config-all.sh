...
echo -en "${BRIGHT_BLUE}Welcome to $DISPLAY_NAME post install setup.${NORMAL}"
...
echo -en "${CYAN}In order to finalize the system configuration, please input the following:
...
CDN hostname [${YELLOW}`hostname`${CYAN}]:${NORMAL}"
	...
	if [ -z $SPHINX_SERVER2 ];then
		...
		if [ "$ADMIN_CONSOLE_PASSWORD" != "$AGAIN_ADMIN_CONSOLE_PASSWORD" ];then
			...
			echo -en "\n${BRIGHT_RED}ERROR: Admin user login password (must be minimum 8 chars and include at least one of each: upper-case, lower-case, number and a special character): ${NORMAL}"
			read -s ADMIN_CONSOLE_PASSWORD
		fi
        done
...
echo -en "${BRIGHT_BLUE}Configuration of $DISPLAY_NAME finished successfully!${NORMAL}\n"
